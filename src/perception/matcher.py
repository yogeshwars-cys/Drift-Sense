...
def stage1_multiscale(search_u8, ref_u8, prior_center, prior_radius, margin=90,
                      ...
                      nms_radius=25):
    """Coarse appearance search. The scale bracket is now supplied by the
    lattice sensor rather than swept blindly, so the reference footprint is
    typically within 1px of truth instead of within 5px."""
    ...
    for scale in np.linspace(*scale_range, n_scales):
        ...
        if sub.shape[0] <= foot + 2 or sub.shape[1] <= foot + 2:
            continue
        ref_small = cv2.resize(ref_u8, (foot, foot), interpolation=cv2.INTER_AREA)
...
def peaks_from_map(cmap, window, foot, top_k=6, nms_radius=25):
    ...
    for _ in range(top_k):
        ...
        if not np.isfinite(score) or score <= -1e8:
            break
        py, px = idx
        out.append((score, float(px + foot // 2 + x_lo), float(py + foot // 2 + y_lo), foot))
        y0s, y1s = max(0, py - nms_radius), min(cm.shape[0], py + nms_radius)
        x0s, x1s = max(0, px - nms_radius), min(cm.shape[1], px + nms_radius)
        cm[y0s:y1s, x0s:x1s] = -1e9
    return out
...
def match(search_u8, ref_u8, elapsed_time_s, twin, model,
          ...
          landmark_z_threshold=4.0, belief_threshold=0.20):
    ...
    res_s = aperiodic_residual(search_u8, SEARCH_PITCH_BAND)
    res_r = aperiodic_residual(ref_u8, REF_PITCH_BAND)
    ...
    emb_anchor = embed(model, [ref_u8])[0]
    embed_sims = embed(model, patches) @ emb_anchor

    nodes = []
    for (score, x, y, foot), esim in zip(valid, embed_sims):
    ...
    node = d['node']
    ref_small = cv2.resize(ref_u8, (node['foot'], node['foot']),
